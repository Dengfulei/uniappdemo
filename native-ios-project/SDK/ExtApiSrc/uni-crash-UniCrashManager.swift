...
//  DCloudUniCrash
//
//  Created by DCloud on 2024/9/13.
//
...
public class UniCrashManager {
    ...
    // 设置最大crash 缓存次数
    private static let maxCrashCaches: Int32 = 5
    private static var crashCacheMap : UniCrashCacheMap<NSNumber, String>?
    
    // 是否是uni统计
    public static var isUniStatistics: Bool = false
    ...
    public class func initCrash() {
        //规则调整，废弃由mannifest.json中UniStatistics参数配置，改为调用uni.__getAppCrashInfo后触发上传到uni统计服务器，否则上传到DCloud服务器
        //uni.__getAppCrashInfo是内部api，被js框架封装到uni.report api中，在uvue框架的app生命周期方法onLaunch中调用
        ...
        crashHandler?.maxReportCount = maxCrashCaches
        ...
        DispatchQueue.global().asyncAfter(deadline: .now() + 1.0) {
            if UniCrashManager.isUniStatistics == false {
                reportCrash()
            }
        }
    }
...
extension UniCrashManager {
    ...
    /*
     ...
     carrierid: 运营商id
     carriername： 运营商名称
     ua: User Agent   "Mozilla/5.0 (iPhone; CPU iPhone OS 18_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 uni-app-x/"
     it: 应用集成方式, 1表示小程序SDK方式集成，0表示独立应用模式。 默认值为0。
     psdk: 是否为离线打包,仅独立引用模式提交此字段（it=0）。默认值为0。
     uv: uniapp版本是1， uniappx 版本是2，
     fv: 客户端uni-app运行环境的版本号
     abis: 设备支持的CPU架构，
     channel: 渠道标识
     */
    ...
    private class func getPreParameters() {
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            preParams.set("fv", UTSiOS.getHxVersion())
            preParams.set("abis", UniDeviceInfo.getCPUArchitecture())
            preParams.set("psdk", UTSiOS.getIsOnline() ? 0 : 1)
            preParams.set("channel", UTSiOS.getChannel())
    ...
    private class func getParametersWithLog(_ content: String? = nil, type: UniCrashType? = .native, jsUrl: String? = nil ) -> [String: Any] {
        ...
        let fv = preParams?.get("fv") ?? UTSiOS.getHxVersion()
        ...
        let abis = preParams?.get("abis") ?? UniDeviceInfo.getCPUArchitecture()
        let psdk = preParams?.get("psdk") ?? (UTSiOS.getIsOnline() ? 0 : 1)
        let channel = preParams?.get("channel") ?? UTSiOS.getChannel()
        let it = 0
